...
origin_localized_items = {
	{'gui-edit.origin-left_top'},
	{'gui-edit.origin-right_top'},
	{'gui-edit.origin-left_bottom'},
	{'gui-edit.origin-right_bottom'}
}
corner_to_index = {
	left_top = 1,
	right_top = 2,
...
local function get_settings_table(player_index)
	return global.tilegrids.registry[util.player_table(player_index).cur_editing].settings
end
...
local function type_switch_state_changed(e)
	...
	tilegrid.refresh(util.player_table(e.player_index).cur_editing)
end
...
function edit_gui.create(parent, player_index, settings, hot_corner)
	...
	event.gui.on_click(
		def_titlebar_flow.add{type='sprite-button', name='tl_edit_def_titlebar_button_confirm', style='green_icon_button', sprite='check_mark',
...
function edit_gui.update(player_index)
	...
	local grid_type = settings.grid_type
	elems.divisor_label.caption = {'gui-edit.'..type_index_to_name[grid_type]..'-divisor-label-caption'}
